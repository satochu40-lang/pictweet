...
RSpec.describe 'ツイート投稿', type: :system do
  before do
    ...
    @tweet_image = 'https://s.eximg.jp/exnews/feed/aichoo/aichoo_201503_1.jpg'
  end
  context 'ツイート投稿ができるとき'do
    it 'ログインしたユーザーは新規投稿できる' do
      ...
      expect(page).to have_selector("div[style*='#{@tweet_image}']")
...
RSpec.describe 'ツイート削除', type: :system do
  ...
  context 'ツイート削除ができるとき' do
    it 'ログインしたユーザーは自らが投稿したツイートの削除ができる' do
      ...
      # 削除後にトップページへ遷移したことを確認する
      expect(page).to have_current_path(root_path)
      # トップページに「削除」ボタンがないことを確認する
...
RSpec.describe 'ツイート詳細', type: :system do
  ...
  it 'ログインしたユーザーはツイート詳細ページに遷移してコメント投稿欄が表示される' do
    ...
    # ツイートに「詳細」へのリンクがある
    all('.more')[0].hover
    expect(page).to have_link '詳細', href: tweet_path(@tweet)
